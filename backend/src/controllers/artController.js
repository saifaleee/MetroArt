...
// Helper function to generate hash from file buffer
...
exports.createArtPiece = async (req, res) => {
  ...
  try {
    // Parse isPublished properly from form data
    let published = true; // Default to true
    if (isPublished !== undefined) {
      // Handle string conversion from form data
      published = isPublished === 'true' || isPublished === true;
    }
    
    console.log('Creating art piece with publish status:', published);
    
    // Generate hash based on the image file content
    const imageBuffer = req.file.buffer;
    const verificationHash = generateImageHash(imageBuffer);
    console.log('Generated verification hash from image:', verificationHash);
    ...
    const artPiece = await ArtPiece.create({
      ...
      verificationHash: verificationHash // Use the image-based hash
    });
...
exports.getArtPieceById = async (req, res) => {
  try {
    ...
    // Check if id is in a valid format to prevent database errors
    if (!req.params.id || req.params.id === 'my-artworks' || req.params.id === 'image') {
      console.log(`Invalid art piece ID format: ${req.params.id}`);
